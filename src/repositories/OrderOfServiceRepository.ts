...
class OrderOfServiceRepository {
  ...
  static async getUnique(cod, tenant_id) {
    ...
    const order_of_service = await connect.query(
      "SELECT * FROM order_of_service WHERE cod_order = $1 AND tenant_id = $2",
      [cod, tenant_id]
    );
    connect.release();
  ...
  static async getAll(tenant_id) {
    const connect = await connection.connect();
    const order_of_service = await connect.query(
      "SELECT * FROM order_of_service WHERE tenant_id = $1",
      [tenant_id]
    );
    connect.release();
    return order_of_service.rows;
  ...
  static async create(created_at, tenant_id) {
    const query =
      "INSERT INTO order_of_service(created_at, tenant_id) VALUES ($1, $2) RETURNING cod_order";

    const values = [created_at, tenant_id];
  ...
  static async removeEstimate(cod, tenant_id, idEstimate) {
    ...
    const query =
      "UPDATE order_of_service SET estimate = $1, value = $2 WHERE cod_order = $3 AND tenant_id = $4";

    const values = [estimateArray, newValue, cod, tenant_id];
    const connect = await connection.connect();
  ...
  static async removeEstimateSimple(cod, tenant_id) {
    ...
    const values = [null, null, cod, tenant_id];
    const connect = await connection.connect();
    const removed = await connect.query(query, values);
    connect.release();
    await this.reloadSocketData(cod, tenant_id);
  ...
  static async updateEstimate(estimateArray, totalPrice, cod, tenant_id) {
    ...
    const values = [estimate, totalPrice, cod, tenant_id];
    const connect = await connection.connect();
  ...
  static async remove(cod_order, tenant_id) {
    ...
    const removed = await connect.query(
      "DELETE FROM order_of_service WHERE cod_order = $1 AND tenant_id = $2",
      [cod_order, tenant_id]
    );
    connect.release();
    return removed.rowCount;
